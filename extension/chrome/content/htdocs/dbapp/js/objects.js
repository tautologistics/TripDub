...
function Relation_update() {
	...
	*/
	...
	eri[0].setAttribute("src","./images/"+imgsrc+".gif");
}
...
function Relation(parent_1, row_1, parent_2, row_2, id) {
	...
	this.e_relation.setAttribute("src","./images/move_cross.gif");
...
function apply_xslt(data, xslt, callback) {
	// TODO: support async loading with callback
	if (document.implementation && document.implementation.createDocument) {				
		var xsl_data = document.implementation.createDocument("", "", null);
		xsl_data.async = !!callback;
		xsl_data.load(xslt); // on async call callback
...
function IOAdmin_go() {
	switch (this._select.value) {
		...
		case "db_out": /* save do db */
			...
			ajax_command(POST,"io.jss",in_ref,out_ref);
		break;
		...
		case "show_keywords": /* Show keywords */
			...
			ajax_command(GET,"io.jss?show=show",function(){return true},out_ref);
		break;
		...
		case "db_import": /* get schema from real database */
			ajax_command(GET,"import.jss?schema=" + schema, function(){ return(true); }, import_xml);
			break;

		case "db_export": /* send schema back to real database */
			ajax_command(POST,"export.jss?schema=" + schema, function () { return(export_json()); }, export_db);
			break;
