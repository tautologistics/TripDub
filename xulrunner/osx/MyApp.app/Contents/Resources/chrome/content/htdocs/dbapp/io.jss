...
/**/
	...
	var action = 0; /* defaultni je export */
	...
	switch (action) {
		case 0:
			var key = ((request.form['key'] != null) ? request.form['key'] : '');
			var data = ((request.form['data'] != null) ? request.form['data'] : '');
			if (!key) {
				throw(new ExBase("Failed to get 'key'"));
			} else {
				/*
